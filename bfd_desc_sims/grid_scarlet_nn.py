...
def generate_grid_catalog_scarlet_nn(args: dict):
    ...
    for igal in range(tot_gal):
        ...
        for i in range(n_gal):
            timages.append(galsim.ImageF(bounds, wcs=galsim_wcs))
        ...
        for iigal in range(n_gal):
            ...
            for iim in range(nimages):
                ...
                if iim == 0:
                    cgal.drawImage(filters[filters_all[iim]], image=timages[iigal], center=(x,y), add_to_image=True )
        ...
        for parentIdx, measParentRecord in enumerate(measParentCat):
            ...
            for child in measChildCat:
                ...
                if use_truth_nn is None:
                    ...
                    truth.image.array[:] = 0
                    for truth_index in range(n_gal):
                        if truth_index == index:
                            continue
                        truth.image.array[:] += texposures[truth_index].image[bbox].array
...
def generate_grid_catalog_write_scarlet_nn(args: dict):
    """Generate catalog and wrie out result.  This is useful in multiprocessing"""
    sim, result, cat = generate_grid_catalog_scarlet_nn(args)
...
def generate_grid_prior_scarlet_nn(args: dict):
