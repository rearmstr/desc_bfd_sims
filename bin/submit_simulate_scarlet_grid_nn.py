...
if args.no_prior is False:
    cmd = f"python bfd_desc_sims/bin/simulate_scarlet_grid_nn.py {args.config_file} run_prior=True run_pqr=False njobs=0 {args.prior_args}"
...
for i, index_list in enumerate(index_lists):
    ...
    for j,sub_list in enumerate(sub_lists):
        cmd += f"python bfd_desc_sims/bin/simulate_scarlet_grid_nn.py {args.config_file} njobs={len(sub_list)} index={index} {args.args}\n"
        index += len(sub_list)
