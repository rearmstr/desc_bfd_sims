...
from bfd_desc_sims import (generate_pqr, read_prior,
                           generate_blend_catalog_scarlet, generate_blend_prior_scarlet)
...
def generate_scarlet_blend_catalog_write(args: dict):
    ...
    try:
        sim, result, cat, exp = generate_blend_catalog_scarlet(args)
...
def generate_scarlet_blend_prior_write(args: dict):
    """Generate catalog and wrie out result.  This is useful in multiprocessing"""
    try:
        sim, result, cat, prior, prior_sel = generate_blend_prior_scarlet(args)
    except:
        return
...
if data['run_prior']:
    for i in range(data['njobs_prior']):
        ...
        if i == 0:
            ...
            sim, result, cat, exps = generate_blend_catalog_scarlet(cat_args)
