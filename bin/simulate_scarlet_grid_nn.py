...
from bfd_desc_sims import (generate_pqr, read_prior,
                           generate_grid_catalog_scarlet_nn, generate_grid_prior_scarlet_nn)
...
def generate_scarlet_grid_catalog_write(args: dict):
    """Generate catalog and wrie out result.  This is useful in multiprocessing"""
    try:
        cat = generate_grid_catalog_scarlet_nn(args)
...
def generate_scarlet_grid_prior_write(args: dict):
    """Generate catalog and wrie out result.  This is useful in multiprocessing"""
    try:
        cat, prior, prior_sel = generate_grid_prior_scarlet_nn(args)
    except Exception as e:
...
if data['run_prior']:
    for i in range(data['njobs_prior']):
        ...
        if i == 0:
            ...
            cat = generate_grid_catalog_scarlet_nn(cat_args)
...
if prior_threads > 1:
    prior_results = prior_pool.map(generate_scarlet_grid_prior_write, prior_list_args)
else:
    for arg in prior_list_args:
        generate_scarlet_grid_prior_write(arg)
...
for i in range(data['njobs']):
    ...
    if(data['run_catalog']):
        ...
        if 'no_catalog' in local_args:
            continue
        function = generate_scarlet_grid_catalog_write
