...
if args.no_prior is False:
    cmd = f"python bfd_desc_sims/bin/simulate.py {args.config_file} njobs=1 n_threads=1 run_prior=True run_pqr=False njobs=0 {args.prior_args}"
...
for i, index_list in enumerate(index_lists):
    time.sleep(1)
    cmd = f"simulate.py {args.config_file} njobs={len(index_list)} index={index} {args.args}"
